    ...
    while(!pq.empty()){
        ...
        for(int i=0;i<4;i++){
            ...
            if(inrange(nextp) && maze[nextp.first][nextp.second]!=1 && !check[nextp.first][nextp.second]){
                ...
                step[nextp.first][nextp.second]=step[nowp.first][nowp.second]+1;
                pq.push(make_pair(heuri(nextp)+step[nextp.first][nextp.second], nextp));
            }
        }
    }
    return ret;
...
//    freopen("output_ex1_ASS.txt", "w", stdout);
    scanf("%d %d",&n,&m);
    for(int i=0;i<n;i++){
        for(int j=0;j<m;j++){
            scanf("%d",&maze[i][j]);
